...
${routes
  .map(({ file }, i) => `import * as $ROUTE${i} from "${removeExtname(file)}";`)
  .join("\n")}
${islands
  .map(
    ({ file }, i) => `import * as $ISLAND${i} from "${removeExtname(file)}";`
  )
  .join("\n")}
${middlewares
  .map(
    ({ file }, i) =>
      `import * as $MIDDLEWARE${i} from "${removeExtname(file)}";`
  )
  .join("\n")}
${
  notFound
    ? `import * as $NOTFOUND from "${removeExtname(notFound.file)}";\n`
    : ""
}
...
export async function dev(base: string, entrypoint: string) {
  ...
  if (manifestChanged)
    await generate(join(dir, "./web-server.gen.ts"), newManifest);
